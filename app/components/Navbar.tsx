  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "py-3 bg-ink-950/70 backdrop-blur-xl border-b border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.45)]"
          : "py-6 bg-transparent"
      }`}
    >
        ...
        <ul className="hidden items-center gap-1 md:flex">
          {links.map((l) => (
            <li key={l.href}>
              ...
              >
                <span>{l.label}</span>
                <span className="pointer-events-none absolute inset-x-4 -bottom-0.5 h-px origin-left scale-x-0 bg-gradient-to-r from-white via-white to-transparent transition-transform duration-500 group-hover:scale-x-100" />
              </a>
            </li>
          ))}
          <li className="ml-3">
            <a
              href="#contacto"
              className="group relative inline-flex overflow-hidden rounded-full border border-white/15 bg-white/[0.03] px-5 py-2 text-sm font-medium tracking-wide text-white shadow-glow transition-all duration-300 hover:bg-white hover:text-black hover:shadow-glow-strong"
            >
